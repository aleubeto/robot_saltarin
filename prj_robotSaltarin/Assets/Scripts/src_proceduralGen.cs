    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if(collision.gameObject.name.StartsWith("obj_platform1"))
        {
            if (probability == 1)
            {
                ...
                Instantiate(springPrefab, new Vector2(Random.Range(-5f, 5f), player.transform.position.y + (12 * Random.Range(0.5f, 0.7f))), Quaternion.identity);
            }
            else if (probability > 1 && probability < 4)
            {
                Destroy(collision.gameObject);
                Instantiate(breakablePrefab, new Vector2(Random.Range(-5f, 5f), player.transform.position.y + (12 * Random.Range(0.5f, 0.7f))), Quaternion.identity);
            }
            else
            {
                collision.gameObject.transform.position = new Vector2(Random.Range(-5f, 5f), player.transform.position.y + (12 * Random.Range(0.5f, 0.7f)));
            }
        }
        else if(collision.gameObject.name.StartsWith("obj_platform2"))
        {
            ...
            {
                Destroy(collision.gameObject);
                Instantiate(platformPrefab, new Vector2(Random.Range(-5f, 5f), player.transform.position.y + (12 * Random.Range(0.5f, 0.7f))), Quaternion.identity);
            }
        }
